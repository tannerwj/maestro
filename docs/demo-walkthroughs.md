...
```bash
make inspect-runs CONFIG=examples/gitlab-claude-auto.yaml
make inspect-state CONFIG=examples/gitlab-claude-auto.yaml
```

8. If you want to reset the local demo state:

```bash
make reset-issue CONFIG=examples/gitlab-claude-auto.yaml ISSUE=group/project#123
...
```bash
make run CONFIG=examples/linear-codex-auto.yaml
```
...
```bash
make inspect-runs CONFIG=examples/linear-codex-auto.yaml
make inspect-state CONFIG=examples/linear-codex-auto.yaml
```

8. If you want to reset the local demo state:

```bash
make reset-issue CONFIG=examples/linear-codex-auto.yaml ISSUE=TAN-123
make cleanup-workspaces CONFIG=examples/linear-codex-auto.yaml
```
...
```bash
make run CONFIG=examples/multi-source-claude-auto.yaml
```

6. Inspect the source-scoped state:

```bash
make inspect-runs CONFIG=examples/multi-source-claude-auto.yaml
make inspect-state CONFIG=examples/multi-source-claude-auto.yaml
```
